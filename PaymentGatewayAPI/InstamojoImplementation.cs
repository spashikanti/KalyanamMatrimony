...
namespace PaymentGatewayAPI
{
     ...
     */
        ...
        private static AccessTokenResponse getAccessToken(string clientId, string clientSecret, string authEndpoint)
		{
            ...
            try
            {
                using (var client = new WebClient())
                {
                    ...
                    objPaymentRequestDetailsResponse = JsonConvert.DeserializeObject<AccessTokenResponse>(responseString); //JsonDeserialize<AccessTokenResponse>(responseString);
        ...
        */
        ...
        private async Task<string> api_call_new(string method, string path, object PostData = null)
        {
            string URL = build_api_call_url(path);
            ...
            //request.Content = new StringContent("OK", Encoding.UTF8, "application/json");

            using (var client = new HttpClient())
            {
                using (var request = new HttpRequestMessage(HttpMethod.Post, URL))
                {
                    using (var httpContent = CreateHttpContent(PostData))
                    {
        ...
        */
        public async Task<CreatePaymentOrderResponse> createNewPaymentRequest(PaymentOrder objPaymentRequest)
        {
            ...
            try
            {
                string stream = await api_call_new("POST", "gateway/orders/", objPaymentRequest);
